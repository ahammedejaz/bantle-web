...
export function Footer() {
  ...
  return (
    <footer className="grain relative isolate overflow-hidden bg-canvas text-canvas-fg">
      {/* A single mint highlight anchors the top edge so the band does not read
          as a flat rectangle where the light body ends. */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -top-40 left-1/2 h-80 w-[70rem] -translate-x-1/2 rounded-full bg-mint/10 blur-[120px]"
      />
      ...
      <div className="container-x relative z-10 pb-10 pt-16 md:pt-20">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1.4fr)_minmax(0,2fr)]">
          <div>
            ...
            <p className="mt-5 max-w-sm text-[15px] leading-[1.7] text-canvas-fg-muted">
              {TAGLINE} A coordination and chat app for household and
              family-plan arrangements in India.
            </p>
            <a
              href={`mailto:${CONTACT_EMAIL}`}
              className="group mt-5 inline-flex items-center gap-1.5 text-[15px] font-medium text-mint transition-colors hover:text-white"
            >
              {CONTACT_EMAIL}
              <ArrowUpRight
            ...
            <div className="mt-8">
              <p className="mb-3 font-mono text-[10px] uppercase tracking-[0.18em] text-canvas-fg-muted/70">
                Get the app
              </p>
          ...
          <div className="grid gap-10 sm:grid-cols-3">
            {FOOTER_GROUPS.map((group) => (
              <div key={group.title}>
                <p className="mb-4 font-mono text-[10px] uppercase tracking-[0.18em] text-canvas-fg-muted/70">
                  {group.title}
                </p>
                <ul className="flex flex-col gap-3">
                  {group.links.map((link) => (
                    <li key={link.href}>
                      <Link
                        href={link.href}
                        className="text-[14.5px] text-canvas-fg-muted transition-colors duration-150 hover:text-canvas-fg"
                      >
                        {link.label}
                      </Link>
                    </li>
        ...
        <div className="mt-14 flex flex-col gap-3 border-t border-canvas-edge/10 pt-7 text-[13.5px] text-canvas-fg-muted/80 md:flex-row md:items-center md:justify-between">
      ...
      <span
        aria-hidden="true"
        className="pointer-events-none absolute -bottom-[0.28em] left-1/2 z-0 w-full -translate-x-1/2 select-none text-center font-display text-[22vw] font-bold leading-none tracking-display text-mint/[0.045]"
      >
        Bantle
      </span>
    </footer>
