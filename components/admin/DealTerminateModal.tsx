...
export interface DealTerminateResponse {
  deal: {
    id: string;
    status: string | null;
...
interface DealTerminateModalProps {
  open: boolean;
  dealId: string;
  listingTitle: string;
  onOpenChange: (open: boolean) => void;
  onTerminated: (response: DealTerminateResponse) => void;
  onError: (message: string) => void;
}

export function DealTerminateModal({
  open,
  dealId,
  listingTitle,
  onOpenChange,
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const response = await fetch(`/admin/api/deals/${dealId}/terminate`, {
        method: "POST",
      ...
      onTerminated(data as DealTerminateResponse);
    } catch (e) {
      ...
      <Dialog.Portal>
        ...
        >
          <div className="flex items-start gap-3 mb-4">
            <div className="flex-1 pt-1">
              <Dialog.Title className="font-serif italic text-xl text-teal-900 leading-tight">
                Force-terminate deal?
              </Dialog.Title>
              <Dialog.Description className="text-sm text-ink-muted mt-2">
                This sets the deal status to cancelled and records an admin
                termination. Existing listing, chat history, ratings, and
                other deals are not changed.
              </Dialog.Description>
            </div>
            <Dialog.Close
              aria-label="Close"
          ...
          <div className="flex gap-2 rounded-card border border-amber-200 bg-amber-50 p-3 mb-4 text-amber-900">
            ...
            <p className="text-sm">
              Both host and buyer will receive a transactional notification.
              This does not close the listing or alter unrelated deals.
            </p>
          </div>

          <div>
            <label
              htmlFor="deal-terminate-reason"
              className="block text-sm font-medium text-teal-900 mb-2"
            >
              Termination reason
            </label>
            <textarea
              id="deal-terminate-reason"
              rows={4}
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              disabled={submitting}
              placeholder="Required. Sent to both parties and recorded in the audit log."
          ...
          <div className="flex gap-2 mt-6">
            ...
            >
              {submitting ? "Terminating..." : "Terminate deal"}
            </button>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
