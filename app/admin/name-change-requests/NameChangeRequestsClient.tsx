...
interface NameChangeRequestListItem {
  id: string;
  user_id: string;
...
export function NameChangeRequestsClient() {
  const toast = useAdminToast();
  const [status, setStatus] = useState<StatusFilter>("pending");
  const [page, setPage] = useState(1);
  const [items, setItems] = useState<NameChangeRequestListItem[]>([]);
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/admin/api/name-change-requests?${params}`,
      );
      if (!response.ok) {
      ...
      const data = (await response.json()) as {
        requests: NameChangeRequestListItem[];
        total: number;
        page_size: number;
      };
      ...
      toast.show(
        error instanceof Error
          ? error.message
          : "Name-change requests could not be loaded.",
        "error",
      );
    } finally {
      setLoading(false);
    ...
    <div>
      ...
      {items.length === 0 && !loading ? (
        <div className="border border-line rounded-card bg-cream-card p-8 text-center text-ink-muted">
          No name-change requests match this filter.
        </div>
      ) : (
        <div className="space-y-3">
          {items.map((item) => (
            <NameChangeRequestRow key={item.id} item={item} />
          ))}
        </div>
      )}
...
function NameChangeRequestRow({
  item,
}: {
  item: NameChangeRequestListItem;
}) {
  const statusDisplay = getStatusDisplay(item.status);

  return (
    <Link
      href={`/admin/name-change-requests/${item.id}`}
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="text-sm text-ink">
            {item.current_display_name ?? "No current name"} &rarr;{" "}
            {item.requested_display_name}
          </p>
          <p className="text-xs text-ink-muted mt-1">
            Requested {fmtDate(item.requested_at)}
