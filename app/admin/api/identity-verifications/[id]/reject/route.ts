...
export async function POST(
  ...
  const imageRetentionUntil = addDaysIso(
    now,
    settings.rejected_selfie_retention_days,
  );
  ...
  if (updateError) {
    const correlationId = safeAdminErrorLog(
      "admin_identity_verification_reject_update_failed",
      updateError,
      { operation: "identity_verification_reject_update" },
    );
    return adminErrorResponse("Verification could not be rejected.", 500, {
      correlationId,
    });
  }
  ...
  const { error: profileError } = await supabase
    ...
    .update({
      ...
      identity_verification_rejected_at: nowIso,
  ...
  if (profileError) {
    const correlationId = safeAdminErrorLog(
      "admin_identity_verification_reject_profile_failed",
      profileError,
      { operation: "identity_verification_reject_profile" },
    );
